...
contract LeagueMaker is ReentrancyGuard, Ownable {
    ...
    event setAdminStateEvent(address userAddr, bool state);
    ...
    struct League {
        ...
        uint256 liveTime;
        uint256 closeTime;
        uint256 minEntry; // minimum fee in wei
        uint256 totalPrize;
    ...
    function setAdminState(address userAddr, bool state) public onlyOwner {
        _isAdmin[userAddr] = state;
        emit setAdminStateEvent(userAddr, state);
    }

    function toggleBlockPlayer(uint256 _leagueId, string memory _player) public onlyAdmin {
        require(_leagues[_leagueId].exists == true, "league doesn't exist");
        require(_leagueParticipants[_leagueId][_player].exists == true, "participant doesn't exist in league");
    ...
    function openLeague(
        ...
        ) public onlyAdmin //only owner can make leagues for now{
        {
            lastLeagueId++;
    ...
    nonReentrant payable{
        ...
        if(msg.sender == owner())
            setAdminState(msg.sender, true); // then emit event that'll set such user 
    }

    // TODO: cancelLeague + cancelUser

    // onlyOwner can start a league
    function liveLeague(uint256 _leagueId) external onlyAdmin {
    ...
    function closeLeague(uint256 _leagueId, string[] memory _winners) external onlyAdmin {
        // status must be live
